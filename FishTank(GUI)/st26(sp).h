...
int st261::max1(int Inta,int Intb)
{
    if(Inta>Intb) return Inta; else return Intb;
}

int st261::min1(int Inta,int Intb)
{
    if(Inta<Intb) return Inta; else return Intb;
}

int st261::abs1(int Inta)
{
    if(Inta>0) return Inta; else return -Inta;
}

int st261::dis(int x1,int y1,int x2,int y2)
{
     return abs1(x1-x2)+abs1(y1-y2);
}

void st261::refreshGraph()
{
    for(int i=1;i<=N;i++)
        for(int j=1;j<=M;j++)
            pic[i][j]=askWhat(i,j);
...
int st261::evalueProfitFrom(int p)
{
...
int st261::evalueDangerRevive(int dx,int dy)  //评估时需注意吃完水藻后风险降低
{
    int sum=0;
    for(int i=1;i<=N;i++)
        for(int j=1;j<=M;j++)
...
int st261::evalueDanger(int dx,int dy)  //评估时需注意吃完水藻后风险降低
{
    int sum=0;
    for(int i=1;i<=N;i++)
        for(int j=1;j<=M;j++)
...
int st261::evalueProfit(int dx,int dy)
{
    int sum=0;
    for(int i=1;i<=N;i++)
        for(int j=1;j<=M;j++)
...
void st261::revive(int &toX,int &toY)
{
