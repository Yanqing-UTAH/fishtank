...
int st01::dist(int x1,int y1,int x2,int y2)
{
    return abs(x1 - x2) + abs(y1 - y2);
}

int st01::ceil(int a,int b)
{
    if (a % b == 0) return a / b;
    else return a / b + 1;
}

int st01::calValue(int x,int y)
{
    int i,j;
    int ans = 0;
...
void st01::calSafe(int x,int y,int& dam1,int& dam2)
{
    int i,j,k;
    int damage = 0;
    bool eat = false;
...
int st01::calSingleValue(int x,int y)
{
    int ans = 0;
...
void st01::decide(int& px,int& py,int& ax,int& ay)
{
    int i,j;
    int x1 = max(1,x - spd);
    int x2 = min(N,x + spd);
...
bool st01::isSafe(int x,int y,int safety)
{
    int i,j,k;
    int damage = 0;
    bool eat = false;
...
void st01::revive(int& rx,int& ry)
{
    int i,j;
    int ans = 0;
    int value = 0;
